...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  `)
    ...
    await db.run(sql`ALTER TABLE \`media\` ADD \`processing_status\` text DEFAULT 'pending';`)
    await db.run(sql`ALTER TABLE \`media\` ADD \`prefix\` text DEFAULT 'images';`)
    await db.run(
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  `)
    ...
    await db.run(sql`ALTER TABLE \`media\` DROP COLUMN \`processing_status\`;`)
